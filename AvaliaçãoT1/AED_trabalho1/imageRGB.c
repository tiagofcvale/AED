...
Image ImageRotate90CW(const Image img) {
  ...
  for (uint32 i = 0; i < rotated->height; i++) {
    ...
    for (uint32 j = 0; j < rotated->width; j++) {
      rotated->image[i][j] = img->image[img->height - 1 - j][i];
      PIXMEM += 2; 
    }
  }
...
Image ImageRotate180CW(const Image img) {
  ...
  for (uint32 i = 0; i < rotated->height; i++) {
    rotated->image[i] = AllocateRowArray(rotated->width);
    for (uint32 j = 0; j < rotated->width; j++) {
      rotated->image[i][j] = img->image[img->height - 1 - i][img->width - 1 - j];
      PIXMEM += 2; 
    }
  }
...
int ImageRegionFillingRecursive(Image img, int u, int v, uint16 label) {
  ...
  if (original_label == label || original_label != WHITE) {
    return 0;
  }
  ...
  for (int d = 0; d < 4; d++) {
    int new_u = u + directions[d][0];
    int new_v = v + directions[d][1];
    if (ImageIsValidPixel(img, new_u, new_v)) {
      count += ImageRegionFillingRecursive(img, new_u, new_v, label);
    }
  }
...
int ImageRegionFillingWithSTACK(Image img, int u, int v, uint16 label) {
  ...
  while (!StackIsEmpty(stack)) {
    ...
    int curr_u = PixelCoordsGetU(current);
    int curr_v = PixelCoordsGetV(current);
    
    if (ImageIsValidPixel(img, curr_u, curr_v) && 
        img->image[curr_v][curr_u] == original_label) {
      
      img->image[curr_v][curr_u] = label;
      PIXMEM += 2; 
      count++;
      
      int directions[4][2] = {{1,0}, {-1,0}, {0,1}, {0,-1}};
      for (int d = 0; d < 4; d++) {
        int new_u = curr_u + directions[d][0];
        int new_v = curr_v + directions[d][1];
        if (ImageIsValidPixel(img, new_u, new_v)) {
          PixelCoords neighbor = PixelCoordsCreate(new_u, new_v);
...
int ImageSegmentation(Image img, FillingFunction fillFunct) {
  ...
  int region_count = 0;
  rgb_t next_color = 0x000000; 
  
  for (uint32 v = 0; v < img->height; v++) {
    for (uint32 u = 0; u < img->width; u++) {
      if (img->image[v][u] == WHITE) {
        next_color = GenerateNextColor(next_color);
        uint16 new_label = LUTAllocColor(img, next_color);
        
        int pixels_filled = fillFunct(img, (int)u, (int)v, new_label);
