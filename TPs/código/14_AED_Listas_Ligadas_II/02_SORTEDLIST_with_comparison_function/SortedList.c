...
List* merge(List* seq_1, List* seq_2) {
  assert(seq_1 != NULL && seq_2 != NULL);
  // Create a new list with the same comparison function
  List* res = ListCreate(seq_1->compare);

  struct _ListNode* n1 = seq_1->head;
  struct _ListNode* n2 = seq_2->head;

  while (n1 != NULL && n2 != NULL) {
    int cmp = seq_1->compare(n1->item, n2->item);
    if (cmp < 0) {
      ListInsert(res, n1->item);
      n1 = n1->next;
    } else if (cmp > 0) {
      ListInsert(res, n2->item);
      n2 = n2->next;
    } else {
      // If equal, insert one and advance both
      ListInsert(res, n1->item);
      n1 = n1->next;
      n2 = n2->next;
    }
