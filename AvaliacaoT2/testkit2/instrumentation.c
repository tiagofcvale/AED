...
///
/// João Manuel Rodrigues, AED, 2023, 2025
...
/// InstrName[0] = "memops";
/// InstrName[1] = "adds";
/// InstrCalibrate();  // Call once, to measure CTU or read it from CTUFILE
/// ...
...
//HIDE
#include "nullio.h"  // to SILENCE all printfs in this file!
//SHOW
...
double cpu_time(void) {
  ...
  // The first argument in gettime could also be CLOCK_REALTIME.
...
/// Get or measure the Calibrated Time Unit (CTU).
/// Run and time a loop of basic memory and arithmetic operations
/// to define a reasonably cpu-independent time unit.
/// Save this calibrated time unit to the CTUFILE, if possible. 
/// If CTUFILE can be read, get CTU from there
/// and bypass the calibration loop entirely.
void InstrCalibrate(void) { ///
  char *origin = "Oops";
  InstrCTU = 0.0;
  FILE *f = fopen(CTUFILE, "r");
  if (f != NULL) {
    if (fscanf(f, "%lf", &InstrCTU) == 1)
      origin = "Loaded from `" CTUFILE "`";
    fclose(f);
  }
  if (InstrCTU == 0.0) {
    const int size = 1<<12;     // 2^12!
    ...
    // Save InstrCTU in hidden file, if possible
    f = fopen(CTUFILE, "w");
    if (f != NULL) {
      fprintf(f, "%.3f\n", InstrCTU);
      fclose(f);
