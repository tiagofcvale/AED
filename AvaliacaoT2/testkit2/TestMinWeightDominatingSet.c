...
//
// Testing the MinWeightDominatingSet function
//
...
int main(void) {
  ...
  // CHECKING
  // The union of the mwdset with the set of neighbors of each mdset vertex MUST
  // BE the vertices set of the graph

  IndicesSet* temp_set = IndicesSetCreateCopy(mwdset);
  // Walking through the elements of the mdset
  int e = IndicesSetGetFirstElem(mwdset);
  assert(e != -1);
  ...
  while ((e = IndicesSetGetNextElem(mwdset)) != -1) {
  ...
  temp_set = IndicesSetCreateCopy(mwdset);
  // Walking through the elements of the mdset
  e = IndicesSetGetFirstElem(mwdset);
  assert(e != -1);
  ...
  printf("*** SWtinyG.txt ***\n");
  printf("Finding a MIN WEIGHTdominating set\n");
