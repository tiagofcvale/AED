...
Graph* GraphGetSubgraph(const Graph* g, IndicesSet* vertSet) {
  ...
  unsigned int v = IndicesSetGetFirstElem(vertSet);
  ...
  while(v != (unsigned int)-1){
    ...
    if(!ListIsEmpty(edges)){
      ...
      for(int i=0; i<ListGetSize(edges); ListMoveToNext(edges), i++){
        ...
        unsigned int adjVertex = edge->adjVertex;
        double weight = edge->weight;
        
        if(IndicesSetContains(vertSet, adjVertex)){
          if(g->isWeighted){
            GraphAddWeightedEdge(new, v, adjVertex, weight);
          }else{
            GraphAddEdge(new, v, adjVertex);
          }
        }
      }
    }
...
IndicesSet* GraphGetSetAdjacentsTo(const Graph* g, unsigned int v) {
  ...
  if(!ListIsEmpty(edges)){
    ...
    for(int i=0; i<ListGetSize(edges); ListMoveToNext(edges), i++){
      ...
      IndicesSetAdd(adjacents_set, adjVertex);
    }
  }
