...
// Retorna o subgrafo induzido pelos vértices em vertSet (e as arestas entre eles).
// Se vertSet contém todos os vértices, devolve uma cópia do grafo.
Graph* GraphGetSubgraph(const Graph* g, IndicesSet* vertSet) {
  ...
  // Adiciona os vértices do conjunto
  int v_idx = IndicesSetGetFirstElem(vertSet);
  while (v_idx != -1) {
    GraphAddVertex(new, (unsigned int)v_idx);
    v_idx = IndicesSetGetNextElem(vertSet);
  }

  // Copia as arestas entre vértices do conjunto
  v_idx = IndicesSetGetFirstElem(vertSet);
  while (v_idx != -1) {

    // Procura o vértice no grafo original
    struct _Vertex search_key;
    search_key.id = (unsigned int)v_idx;
    ListSearch(g->verticesList, &search_key);
    ...
    if (!ListIsEmpty(edges)) {
      ...
      for (unsigned int i = 0; i < numEdges; i++) {
        ...
        unsigned int neighbor_id = e->adjVertex;

        // Só adiciona se o vizinho também estiver no conjunto
        if (IndicesSetContains(vertSet, neighbor_id)) {

          // Evita duplicados em grafos não orientados: adiciona apenas quando
          // for digraph ou id_origem < id_destino
          if (g->isDigraph || (unsigned int)v_idx < neighbor_id) {
            
            if (g->isWeighted) {
              GraphAddWeightedEdge(new, (unsigned int)v_idx, neighbor_id, e->weight);
            } else {
              GraphAddEdge(new, (unsigned int)v_idx, neighbor_id);
            }
          }
        }
...
IndicesSet* GraphGetSetAdjacentsTo(const Graph* g, unsigned int v) {
  ...
  // "Dummy" com o id para procurar o vértice
  struct _Vertex search_dummy;
  search_dummy.id = v;

  // Procurar o vértice na lista (cast necessário para mover o iterador)
  ListSearch((List*)g->verticesList, &search_dummy);
  ...
  // Se não houver arestas, devolve o conjunto vazio
  if (ListIsEmpty(edges)) {
    return adjacents_set;
  }

  // Percorre as arestas e adiciona os vértices adjacentes
  ListMoveToHead(edges);
  unsigned int numEdges = ListGetSize(edges);
...
double* GraphComputeVertexWeights(const Graph* g) {
  ...
  // Inicializa com -1 (vértice inexistente)
  ...
  ListMoveToHead(vertices);
  unsigned int numVertices = ListGetSize(vertices);
  ...
  for (unsigned int i = 0; i < numVertices; i++) {
    ...
    if (!ListIsEmpty(edges)) {
      // Soma os pesos (ou conta arestas se não houver pesos)
      ListMoveToHead(edges);
      unsigned int numEdges = ListGetSize(edges);
    ...
    // Armazena o peso no índice do vértice
    weightsArray[v->id] = totalWeight;
